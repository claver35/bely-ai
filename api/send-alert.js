...
<body style="margin:0;padding:0;background:#0f1117;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif">
  <div style="max-width:560px;margin:0 auto;padding:40px 20px">
    <div style="margin-bottom:32px">
      <span style="font-size:20px;font-weight:800;color:#fff">BELY <span style="background:linear-gradient(90deg,#00d4ff,#00b3ee);-webkit-background-clip:text;-webkit-text-fill-color:transparent">AI</span></span>
    </div>
    <div style="background:#161820;border:1px solid ${riskColor}40;border-radius:12px;padding:28px;margin-bottom:24px">
      ...
      <div style="font-size:14px;color:#c2c0b6;margin-bottom:24px">A ${riskLevel.toLowerCase()}-risk order was detected in your store <strong style="color:#fff">${shop}</strong></div>
      ...
      <div style="margin-bottom:20px">
        ...
        ${riskReasons.map(r => `<div style="font-size:13px;color:#fca5a5;padding:6px 10px;background:rgba(255,34,34,0.07);border-left:2px solid #ff2222;border-radius:4px;margin-bottom:6px">⚠ ${r}</div>`).join('')}
      </div>` : ''}
      <a href="https://bely-ai.vercel.app/dashboard.html" style="display:block;text-align:center;padding:12px 24px;background:linear-gradient(90deg,#00d4ff,#0099cc);color:#fff;font-weight:700;font-size:14px;border-radius:8px;text-decoration:none">View Dashboard →</a>
    </div>
    <div style="font-size:12px;color:#5F5E5A;text-align:center">
      ...
      <a href="https://bely-ai.vercel.app/dashboard.html" style="color:#5F5E5A">Manage alerts</a>
    </div>
  </div>
</body>
</html>`;
  ...
  try {
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'BELY AI <alerts@belyai.com>',
        to: [to],
